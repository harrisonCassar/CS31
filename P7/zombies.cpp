...
bool Arena::determineNewPosition(int& r, int& c, int dir) const
{
    ...
    {
      case UP:
        if (r-1 < 1) //indicates going out of boundaries
          return false;
        r--;
        break;

      case DOWN:
        if (r+1 > m_rows) //indicates going out of boundaries
          return false;
        r++;
        break;

      case LEFT:
        if (c-1 < 1) //indicates going out of boundaries
          return false;
        c--;
        break;

      case RIGHT:
        if (c+1 > m_cols) //indicates going out of boundaries
          return false;
        c++;
        break;
